<%@ page language="java" contentType="text/html; charset=ISO-8859-1"
	pageEncoding="ISO-8859-1"%>
<!DOCTYPE html>
<html>
<head><link rel = "stylesheet" href = "PrivacyPolisy.css">
<link rel = "stylesheet" href = "header&footer.css">
<meta charset="ISO-8859-1">
<title>Mind Care/Privacy Policy</title>
</head>
<body>
<!--header-->
        <header>
            <div class="navbar">
                <div class="logo">
                    <img src="images/logo2.jpeg" alt="Logo" width="250px" height="100px">  
                </div>
                <ul class="links">
                    <li class="nav"><a class="nav_a" href="home.jsp"><b>Home</b></a></li>
                    <li class="nav">
                    	<a class="nav_a" href="#"><b>Discover</b></a>
                    	<ul class="submenu">
                            <li><a href="Doctor.jsp">Doctors</a></li>
                            <li><a href="#">Diseases</a></li>
                        </ul>
                    	
                    </li>
                    
                    <li class="nav">
                        <a class="nav_a" href=""><b>Get help </b></a>
                        <ul class="submenu">
                            <li><a href="#">Appointment scheduling</a></li>
                            <li><a href="ticketSubmit.jsp">Ticket</a></li>
                            <li><a href="FeedbackInsert.jsp">Feedback</a></li>
                        </ul>
                    </li>
                    <li class="nav"><a class="nav_a" href="aboutUs.jsp"><b>About Us</b></a></li>
                </ul>
                <div class="shortcut">
                    <a href="" class="btn_type1">Login</a>

                    <div class="profile-img">
                        <a href="" ><img src="images/profile.png" width="30px" height="30px" ></a>
                    </div>
                </div>

            </div>
        </header>
        <!--end-->        
        <br><br><br><br><br><br>
        
	<div class = "AA">
		<h1 class="Privacy">
			<u>Privacy Policy</u>
		</h1>
	<br>

		<p>Welcome to the web site of MINDCARE. At MINDCARE, we are
			committed to protecting your privacy and the confidentiality of your
			personal information. This Privacy Policy outlines how we collect,
			use, and safeguard your personal data when you visit our website.</p>

		<ol>
			<li>Information We Collect<br>
				<UL>
					<LI>Personal Information : This includes information such as
						your name, email address, phone number, and any other information
						you provide to us when using our website. You may provide this
						information when scheduling an appointment, using contact forms,
						or signing up for newsletters.</LI>
					<LI>Non-Personal Information: This type of information
						includes data such as your IP address, browser type, operating
						system, and website usage data. We collect non-personal
						information to enhance the functionality and content of our
						website.</LI>
					<LI>Usage Information : We may collect non-personally
						identifiable information about your usage of our portal, such as
						IP addresses, device information, browser type, and cookies, to
						improve our services and enhance user experience.</LI>
				</UL>
			</li>
			<br>
			<li>Use of Information<br>
				<UL>
					<LI>Providing Services : We use the collected information to
						provide medical services, facilitate appointments, deliver test
						results, and offer personalized health care recommendations.</LI>
					<LI>Communication : We may use your contact information to
						send important updates, appointment reminders, and other relevant
						notifications related to your medical care.</LI>
					<LI>Improving Services : We may analyze usage patterns and
						feedback to improve our services, enhance user experience, and
						develop new features.</LI>
				</UL>
			</li>
			<br>
			<li>Information Sharing<br>
				<UL>
					<LI>Service Providers : We may share your information with
						trusted third-party service providers who assist us in delivering
						our services, such as hosting, data analysis, and customer
						support.</LI>
					<LI>Health Professionals : We may share your health
						information with health care providers involved in your treatment
						to ensure the continuity and quality of care.</LI>
					<LI>Legal Requirements : We may disclose your information if
						required by law, court order, or government request, or to protect
						our legal rights and the rights of our users.</LI>
				</UL>
			</li>
			<br>
			<li>Data Security<br>
				<UL>
					<LI>We employ appropriate technical and organizational
						measures to safeguard your information against unauthorized
						access, disclosure, alteration, or destruction.</LI>
					<LI>We use secure data transmission protocols and encrypt
						sensitive information to ensure its confidentiality during
						transmission.</LI>
					<LI>Despite our best efforts, no data transmission or storage
						method is completely secure. While we strive to protect your
						information, we cannot guarantee absolute security.</LI>
				</UL>
			</li>
			<br>
			<li>Third-Party Web sites<br>
				<UL>
					<LI>Our portal may contain links to third-party web sites. We
						are not responsible for the privacy practices or content of those
						web sites. We encourage you to review the privacy policies of any
						third-party sites before providing any personal information.</LI>
				</UL>
			</li>

		</ol>
		<br>
	</div>
	
	<br><br>
	
	<!--Footer-->
        <footer>
            <div class="footer_info">
                <div class="footer_width about">
                    <h2>About</h2>
                    <p>
                        MindCare is a web application that provides informations about psychology and mainly focus on customer care services regarding to user needs.
                    </p>
                    <div class="social_media">
                        <ul>
                            <li><a href="#"><img src="data:image/png;base64,"/></a></li>
                            <li><a href="#"><img src="data:image/png;base64,"/></a></li>
                            <li><a href="#"><img src="data:image/png;base64,"/></a></li>
                        </ul>
                    </div>
                </div>

                <div class="footer_width link">
                    <h2>Quick Links</h2>
                    <ul>
                        <li><a href="home.jsp">Home</a></li>
                        <li><a href="#">Discover</a></li>
                        <li><a href="PrivasyPolisy.jsp">Privacy Policy</a></li>
                        <li><a href="aboutUs.jsp">About Us</a></li>
                    </ul>
                </div>

                <div class="footer_width contact">
                    <h2>Contact</h2>
                    <ul>
                        <li>
                            <span><img src="data:image/png;base64,"/></span>
                            <p>
                                26985 Brighton Lane, Lake Forest, CA 92630
                            </p>
                        </li>

                        <li>
                            <span><img src="data:image/png;base64,"/></span>
                            <a href="#">John@sajadhesami.com</a>
                        </li>

                        <li>
                            <span><img src="data:image/png;base64,"/></span>
                            <a href="#">+30-2106019311</a>
                        </li>
                    </ul>
                </div>
            </div>
            <hr>
            <div class="copy-right">
                <p>
                    Copyright and copyMindCare 2023 All Rights Reserved.
                </p>
            </div>
        </footer>
    <!--end-->
	
</body>
</html>