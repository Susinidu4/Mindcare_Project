<%@ page language="java" contentType="text/html; charset=ISO-8859-1"
	pageEncoding="ISO-8859-1"%>
<!DOCTYPE html>
<html>
<head>
<link rel="stylesheet" href="Doctor.css">
<link rel="stylesheet" href="header&footer.css">
<meta charset="ISO-8859-1">
<title>Mind Care/Doctors</title>
</head>
<body>
	<!--header-->
        <header>
            <div class="navbar">
                <div class="logo">
                    <img src="images/logo2.jpeg" alt="Logo" width="250px" height="100px">  
                </div>
                <ul class="links">
                    <li class="nav"><a class="nav_a" href="home.jsp"><b>Home</b></a></li>
                    <li class="nav">
                    	<a class="nav_a" href="#"><b>Discover</b></a>
                    	<ul class="submenu">
                            <li><a href="Doctor.jsp">Doctors</a></li>
                            <li><a href="#">Diseases</a></li>
                        </ul>
                    	
                    </li>
                    
                    <li class="nav">
                        <a class="nav_a" href=""><b>Get help </b></a>
                        <ul class="submenu">
                            <li><a href="#">Appointment scheduling</a></li>
                            <li><a href="ticketSubmit.jsp">Ticket</a></li>
                            <li><a href="FeedbackInsert.jsp">Feedback</a></li>
                        </ul>
                    </li>
                    <li class="nav"><a class="nav_a" href="aboutUs.jsp"><b>About Us</b></a></li>
                </ul>
                <div class="shortcut">
                    <a href="" class="btn_type1">Login</a>

                    <div class="profile-img">
                        <a href="Profile.jsp" ><img src="images/profile.png" width="30px" height="30px" ></a>
                    </div>
                </div>

            </div>
        </header>
        <!--end-->
        
        <br><br><br><br><br><br>
        
	<h1 class="spe">
		<span class="spa">Psychiatrist Details</span>
	</h1>

	<div class="container">
		<div class="card">
			<div class="card-image">
				<img src="images/M1.jpg" alt="image">
			</div>

			<div class="card-content">
				<h2 class="blog-title">Dr.DAHAM DE SILVA JAYASINGHE</h2>
				<br>
				<p class="excerpt">
				<ul>
					<li>Dr. Neville Fernando Teaching Hospital NFTH</li>
					<br>
					<li>No: 3, Alfred Place, Colombo 03.</li>
					<br>
					<li>0112 407600</li>
				</ul>
				</p>

			</div>
		</div>
	</div>
	<div class="container">
		<div class="card">
			<div class="card-image">
				<img src="images/M2.jpeg" alt="image">
			</div>

			<div class="card-content">
				<h2 class="blog-title">Dr.JAGATH BANDARA</h2>
				<br>
				<p class="excerpt">
				<ul>
					<li>Hemas Hospital Thalawathugoda</li>
					<br>
					<li>18/A, Muhandiram E.D. Dabare Mawatha, Colombo 05.</li>
					<br>
					<li>0117 888888</li>
				</ul>
				</p>

			</div>
		</div>
	</div>
	<div class="container">
		<div class="card">
			<div class="card-image">
				<img src="images/M3.jpeg" alt="image">
			</div>

			<div class="card-content">
				<h2 class="blog-title">MRS. J.A. TEENA MISHEL</h2>
				<br>
				<p class="excerpt">
				<ul>
					<li>Nawaloka Hospital</li>
					<br>
					<li>Miriswattha road, Gampaha, Sri Lanka</li>
					<br>
					<li>0112 304444</li>
				</ul>
				</p>

			</div>
		</div>
	</div>
	<div class="container">
		<div class="card">
			<div class="card-image">
				<img src="images/M4.jpg" alt="image">
			</div>

			<div class="card-content">
				<h2 class="blog-title">Dr. JAYAMAL DE SILVA</h2>
				<br>
				<p class="excerpt">
				<ul>
					<li>Lanka Hospitals</li>
					<br>
					<li>578, Elvitigala Mawatha, Narahenpita, Colombo 5, Sri
						Lanka.</li>
					<br>
					<li>0112 551410, 0115 430000</li>
				</ul>
				</p>

			</div>
		</div>
	</div>
	<div class="container">
		<div class="card">
			<div class="card-image">
				<img src="images/M5.jpg" alt="image">
			</div>

			<div class="card-content">
				<h2 class="blog-title">MRS. S.C.A. ARAMBEPOLA</h2>
				<br>
				<p class="excerpt">
				<ul>
					<li>Lifecare Hospitals (Pvt) Limited</li>
					<br>
					<li>129, S.De.S. Jayasinghe Mawatha, Kohuwala</li>
					<br>
					<li>0212 261500</li>
				</ul>
				</p>

			</div>
		</div>
	</div>

<!--Footer-->
        <footer>
            <div class="footer_info">
                <div class="footer_width about">
                    <h2>About</h2>
                    <p>
                        MindCare is a web application that provides informations about psychology and mainly focus on customer care services regarding to user needs.
                    </p>
                    <div class="social_media">
                        <ul>
                            <li><a href="#"><img src="data:image/png;base64,"/></a></li>
                            <li><a href="#"><img src="data:image/png;base64,"/></a></li>
                            <li><a href="#"><img src="data:image/png;base64,"/></a></li>
                        </ul>
                    </div>
                </div>

                <div class="footer_width link">
                    <h2>Quick Links</h2>
                    <ul>
                        <li><a href="home.jsp">Home</a></li>
                        <li><a href="#">Discover</a></li>
                        <li><a href="PrivasyPolisy.jsp">Privacy Policy</a></li>
                        <li><a href="aboutUs.jsp">About Us</a></li>
                    </ul>
                </div>

                <div class="footer_width contact">
                    <h2>Contact</h2>
                    <ul>
                        <li>
                            <span><img src="data:image/png;base64,"/></span>
                            <p>
                                26985 Brighton Lane, Lake Forest, CA 92630
                            </p>
                        </li>

                        <li>
                            <span><img src="data:image/png;base64,"/></span>
                            <a href="#">John@sajadhesami.com</a>
                        </li>

                        <li>
                            <span><img src="data:image/png;base64,"/></span>
                            <a href="#">+30-2106019311</a>
                        </li>
                    </ul>
                </div>
            </div>
            <hr>
            <div class="copy-right">
                <p>
                    Copyright and copyMindCare 2023 All Rights Reserved.
                </p>
            </div>
        </footer>
    <!--end-->
    
</body>
</html>